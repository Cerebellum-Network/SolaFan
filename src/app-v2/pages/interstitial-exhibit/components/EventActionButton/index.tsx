...
import {EXHIBIT_NFTS_ELEMENT_ID} from '../../interstitial-exhibit-page-view';

export const useStyles = makeStyles<Theme, {finished: boolean}>((theme) => ({
  actionButton: {
    width: '100%',
    ...
    <>
      {!isContentDrop && eventTimelineStatus !== EventTimelineStatus.FINISHED && (
        <Button
          ...
          color="default"
          className={cx(
            GoogleAnalyticsId.UnlockExperienceBtn,
            `${isLocked ? styles.disabledButton : ''} ${styles.actionButton}`,
            buttonText.length > 20 && styles.longText,
          )}
