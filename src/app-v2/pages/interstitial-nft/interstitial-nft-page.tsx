...
import {selectNftById, selectNfts, selectNftsByArtist} from '../../redux/modules/nfts/selectors';
...
import {InterstitialNftPageView} from './interstitial-nft-page-view';
...
const mapStateToProps = (state: any, {nftId}: any) => {
  ...
  return {
    ...
    nftsListing: [...selectNftOrders(state)].filter((order) => !order.cancelled),
    isLoadingNftsCollectibles: false,
    nftsCollectables: Object.values(selectNftsByArtist(state, nft?.creatorId)).filter((nft) => nft != null) as Exclude<
...
export const NftPageWrapper = connect(mapStateToProps, mapDispatchToProps)(InterstitialNftPageView);

export const InterstitialNftPage = () => {
