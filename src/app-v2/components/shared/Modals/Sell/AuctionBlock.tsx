...
export const AuctionBlock = ({
  ...
  const [auctionStartingPrice, setAuctionStartingPrice] = useState<number>(startingPrice);

  const handleBidChange = (e: ChangeEvent<HTMLInputElement>) => {
    const nftPrice = Number(e.target.value);
    if (!nftPrice || nftPrice < startingPrice) {
      setError(true);
    } else {
      setError(false);
    }
    setAuctionStartingPrice(nftPrice);
  };

  const handleSell = useCallback(() => {
    setProcessing(true);
    onSellClick(transformPriceToTokens(auctionStartingPrice), quantity);
  }, [onSellClick, quantity, auctionStartingPrice]);
  ...
  return (
    <>
      <Box className={styles.fieldsBox}>
        ...
        <Box className={styles.field}>
          <TextField
            ...
            variant="outlined"
            inputProps={{type: 'number'}}
            label={t('Minimal acceptable bid')}
            value={transformTokensToPrice(auctionStartingPrice)}
      ...
      {hasError && (
        <Box pt="16px">
          <ErrorMsg variant="error" text={`${t('Minimal acceptable bid has to be more than')} ${startingPrice}`} />
        </Box>
      )}
